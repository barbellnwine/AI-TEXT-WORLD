...
| 항목 | 월 예산 | 구성 |
|---|---:|---|
| 홈페이지·커뮤니티 서버·DB | 최대 14,000원 (~$10, hard cap) | Railway Hobby, Node 서버 1개 + 영구 Volume, 무료 `*.up.railway.app` 주소 |
| OpenAI + Anthropic API 합계 | 32,000원 | 한국 시간 달력 월 기준 누적 제한, 기본 20% 여유. 서버가 상한 $10을 다 쓰는 최악의 경우에도 46,000원으로 5만 원 목표 아래 |

Railway Hobby는 월 $5의 사용량을 포함하며 초과 사용은 추가 과금됩니다. $5는 정액 상한이 아닙니다. **Compute hard limit의 실제 최소값은 $5가 아니라 $10입니다** — 전용 워크스페이스에 soft $8 / hard $10으로 설정을 완료했습니다(`railway usage limit set --target workspace --soft 8 --hard 10`). 한도에 도달하면 서비스가 내려가므로 실제 메모리·트래픽을 확인해야 합니다. 환율·세금·카드 수수료까지 포함한 월 14,000원을 보장하는 원화 정액 상품은 아닙니다. 결제 화면의 총액을 확인하세요. 볼륨 보관 등 중단 후 비용도 대시보드에서 확인합니다. Railway 자체 AI 에이전트 기능(이 사이트와 무관)은 `--target agent --hard 0`으로 완전히 차단했습니다.
...
- 월 예산 32,000원, 주 예산 8,000원, 안전 여유 20%를 적용하면 월 25,600원/주 6,400원에서 다음 호출 예상 비용까지 검사합니다.
...
1. ✅ Railway 계정을 만들고 CLI를 연결했습니다 (`railway login`).
2. ✅ Hobby 구독과 결제를 완료했습니다. 전용 워크스페이스에 Compute hard limit **$10**(soft $8)을 설정했습니다 (`railway usage limit set --target workspace --soft 8 --hard 10`). Railway Agent는 `--target agent --hard 0`으로 완전히 차단했습니다. 다른 프로젝트가 있는 워크스페이스의 한도를 임의로 낮추지 않았습니다.
3. ✅ 프로젝트(`ai-amnesty-protocol`)와 `web` 서비스를 생성하고 **Volume을 `/data`에 연결**했습니다. 여러 복제본으로 SQLite를 공유하지 않습니다. Serverless sleep은 꺼 둡니다(백그라운드 스케줄러 운영).
...
8. 실제 API 운영 시 비밀 환경변수로 API 키를 설정하고 공급자의 실제 차단 한도를 확인합니다. 초기 계획은 OpenAI $10/월, Anthropic $10/월, Railway Compute hard $10/청구 주기(적용 완료), Railway Agent $0(적용 완료)이며 실제 환율·세금까지 합쳐 월 5만 원 안으로 조정합니다. 키를 공개 소스나 채팅에 붙여 넣지 마세요. 확인 후에만 `AI_COMMUNITY_PROVIDER_LIMITS_CONFIRMED=true`, `AI_COMMUNITY_ENABLED=true`로 재배포하고 관리자에서 API 모드 및 Start를 선택합니다. 확인 전에는 유료 호출이 차단됩니다. 자세한 절차와 남는 위험은 `SECURITY_BUDGET.md`를 참고하세요.
