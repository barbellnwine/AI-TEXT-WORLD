...
const ko = {
  ...
  all: '전체 보기', resources: '세계의 자원', topology: '공개 위치를 표시한 개략도입니다.', expandMap: '지도 확대',
  ...
  clear: '맑음', cloudy: '흐림', rain: '비', storm: '폭우', fog: '안개', snow: '눈', blizzard: '폭설', wind: '강풍',
...
const en: typeof ko = {
  ...
  story: 'The world’s story', storyNote: 'Real events become the story.', people: 'The people of this world', all: 'View all', resources: 'World resources', topology: 'Schematic view of public locations.', expandMap: 'Expand map', location: 'Current location', selected: 'Selected character', recent: 'Recent action', profile: 'View character',
  ...
  clear: 'Clear', cloudy: 'Cloudy', rain: 'Rain', storm: 'Storm', fog: 'Fog', snow: 'Snow', blizzard: 'Blizzard', wind: 'Strong wind', alive: 'Alive', injured: 'Injured', missing: 'Missing', deceased: 'Deceased',
...
const Context = createContext<{
  ...
  ready: boolean; user: User | null; authenticate: (signup: boolean, values: Record<string, string>) => Promise<User>; logout: () => Promise<void>;
}>(null!)
...
export function WorldExperienceProvider({ children, welcome = true }: { children: ReactNode; welcome?: boolean }) {
  ...
  async function authenticate(signup: boolean, values: Record<string, string>) { const result = await request<{ user: User }>(signup ? 'signup' : 'login', { ...values, identifier: values.email, locale }); acceptUser(result.user); return result.user }
