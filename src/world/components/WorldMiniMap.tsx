  ...
  return <>
    <div className="world-minimap island-minimap">
      ...
      <svg viewBox="0 0 410 330" role="group" aria-label={t('map')}>
        <defs>
          <radialGradient id={`${id}-land`}><stop stopColor="#50684b" /><stop offset="1" stopColor="#263f35" /></radialGradient>
        ...
        {!designed && <g aria-hidden="true">
          ...
          <path d={island} fill={`url(#${id}-land)`} className="island-coast" />
          <g clipPath={`url(#${id}-clip)`} className="island-contours">
    ...
    <div className="minimap-legend"><span><i />{t('characters')}</span><span><i className="selected-dot" />{t('selected')}</span>{selectedAgentId && <button type="button" onClick={() => onSelectAgent(null)}>{t('clearSelection')}</button>}</div>
