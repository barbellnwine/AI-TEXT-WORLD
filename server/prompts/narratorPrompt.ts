...
//
// WHERE A REAL MODEL CALL GOES: server/domain/narrator.ts's mockNarrator.narrateFallbackScene
// currently assembles plain text with no model call. Replace that function body with a call that
// sends buildNarratorPrompt(events, ...) to a model and parses the prose back out — but keep
// requiring sourceEventIds in the result (see NarratorOutput below) so every scene stays traceable
// to the events that justify it.
...
export function buildNarratorPrompt(events: WorldEvent[], placesById: Map<string, Place>, agentsById: Map<string, Agent>): string {
  ...
  return [
    ...
    '[CONFIRMED EVENTS]',
    JSON.stringify(events.map(publicEventView), null, 2),
    '[PLACE NAMES]',
    JSON.stringify(placeNames, null, 2),
    '[AGENT NAMES]',
    JSON.stringify(agentNames, null, 2),
  ].join('\n\n')
}
